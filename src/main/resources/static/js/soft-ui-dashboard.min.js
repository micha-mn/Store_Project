...
function sidebarType(e) {
	for(var t=e.parentElement.children, n=e.getAttribute("data-class"), i=[], a=0; a<t.length; a++)t[a].classList.remove("active"),
...
total.forEach(function(s, e) {
		var l=document.createElement("div"), t=s.querySelector("li:first-child .nav-link").cloneNode(); t.innerHTML="-", l.classList.add("moving-tab", "position-absolute", "nav-link"), l.appendChild(t), s.appendChild(l), s.getElementsByTagName("li").length; l.style.padding="0px", l.style.width=s.querySelector("li:nth-child(1)").offsetWidth+"px", l.style.transform="translate3d(0px, 0px, 0px)", l.style.transition=".5s ease", s.onmouseover=function(e) {
			let t=getEventTarget(e), a=t.closest("li"); if(a) {
				let n=Array.from(a.closest("ul").children), i=n.indexOf(a)+1; s.querySelector("li:nth-child(" +i+") .nav-link").onclick=function() {
					l=s.querySelector(".moving-tab"); let e=0; if(s.classList.contains("flex-column")) {
						for(var t=1; t<=n.indexOf(a); t++)e+=s.querySelector("li:nth-child(" +t+")").offsetHeight; l.style.transform="translate3d(0px," +e+"px, 0px)", l.style.height=s.querySelector("li:nth-child(" +t+")").offsetHeight
					}

					else {
						for(t=1; t<=n.indexOf(a); t++)e+=s.querySelector("li:nth-child(" +t+")").offsetWidth; l.style.transform="translate3d(" +e+"px, 0px, 0px)", l.style.width=s.querySelector("li:nth-child(" +i+")").offsetWidth+"px"
					}
				}
			}
		}
...
window.addEventListener("resize", function(e) {
		total.forEach(function(n, e) {
				n.querySelector(".moving-tab").remove(); var i=document.createElement("div"), a=n.querySelector(".nav-link.active").cloneNode(); a.innerHTML="-", i.classList.add("moving-tab", "position-absolute", "nav-link"), i.appendChild(a), n.appendChild(i), i.style.padding="0px", i.style.transition=".5s ease"; let s=n.querySelector(".nav-link.active").parentElement; if(s) {
					let e=Array.from(s.closest("ul").children); a=e.indexOf(s)+1; let t=0; if(n.classList.contains("flex-column")) {
						for(var l=1; l<=e.indexOf(s); l++)t+=n.querySelector("li:nth-child(" +l+")").offsetHeight; i.style.transform="translate3d(0px," +t+"px, 0px)", i.style.width=n.querySelector("li:nth-child(" +a+")").offsetWidth+"px", i.style.height=n.querySelector("li:nth-child(" +l+")").offsetHeight
					}

					else {
						for(l=1; l<=e.indexOf(s); l++)t+=n.querySelector("li:nth-child(" +l+")").offsetWidth; i.style.transform="translate3d(" +t+"px, 0px, 0px)", i.style.width=n.querySelector("li:nth-child(" +a+")").offsetWidth+"px"
					}
				}
